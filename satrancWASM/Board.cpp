...
int Board::minimax(int x, int y, int depth, bool max, int tmpX, int tmpY){
	if(depth == 0){
		return this->getPieceValue(x,y);
	}
	int val, tmpVal;
	bool terminal = true;
	if(max){
		...
		for(int j=0;j<8;j++){
			for(int i=0;i<8;i++){
				if(this->pieces[i][j].getColor() != max && this->pieces[i][j].getType() != 0 && !(i == tmpX && j==tmpY)){
					switch(this->pieces[i][j].getType()){
						case 1:
							if(this->movablePawn(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1, false, x, y));
								terminal = false;
							}
							break;
						case 2:
							if(this->movableKnight(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1,false, x, y));
								terminal = false;
							}
							break;
						case 3:
							if(this->movableBishop(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1,false, x, y));
								terminal = false;
							}
							break;
						case 4:
							if(this->movableRook(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1, false, x, y));
								terminal = false;
							}
							break;
						case 5:
							if(this->movableQueen(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1, false, x, y));
								terminal = false;
							}
							break;
						case 6:
							if(this->movableKing(i,j,x,y)){
								val = std::max(val, minimax(i,j,depth-1, false, x, y));
								terminal = false;
							}
							break;
			...
			for(int i=0;i<8;i++){
				if(this->pieces[i][j].getColor() != max && this->pieces[i][j].getType() != 0 && !(i == tmpX && j==tmpY)){
					switch(this->pieces[i][j].getType()){
						case 1:
							if(this->movablePawn(i,j,x,y)){
								val = std::min(val, minimax(i,j,depth-1, false, x, y));
								terminal = false;
							}
							break;
						case 2:
	...
	if(terminal && this->pieces[x][y].getType() != 0){
		return this->getPieceValue(x,y);
	}
	return val;
}

int Board::getPieceValue(int i, int j){
	Piece selectedPiece = this->pieces[i][j];
	...
	return selectedPiece.getColor() ? val : -val;
}
				...
				if (this->pieces[i][j].getColor() != this->pieces[kingX][kingY].getColor()) {
					...
					case 3:
						if (movableBishop(i, j, kingX, kingY)) {
							...
							else { // i < kingX | j > kingY
								if (j > kingY) {
									for (int y = j; y > kingY; y--) {
										tmpList.push_back(Vec2(i + j - y, y));
									}
								}
								else if (j < kingY) { // i < kingX | j < kingY
									for (int y = j; y < kingY; y++) {
										tmpList.push_back(Vec2(i + y - j, y));
									}
								}
...
bool Board::movablePawn(int x, int y, int x2, int y2) {
	...
	if (selectedPiece.getColor()) { // TODO
		if (y == 1) {
...
bool Board::movableBishop(int x, int y, int x2, int y2) {
	...
	if (b) {
		if (y > y2) {
			...
			for (int j = y + 1; j < y2; j++) {  // x1:2 y1:0 - x2:0 y2:2
